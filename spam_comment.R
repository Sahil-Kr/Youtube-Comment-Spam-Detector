...
# Run Naive Bayes on newly extraced field from the Youtube data

SpmModel <- naiveBayes(CLASS~ spm_cnt+is_http+fltrwordcnt+spmtowrd+maxlen, data = YTDSPAM_TRN,laplace = 0.001)
...
#newdata=ytdspam_shakira
prediction <- predict(SpmModel, ytdspam_shakira)
#View(newdata)
print(prediction)
